...
impl Database {
    ...
    fn save_to_file(&self) -> std::io::Result<()> {
        ...
        let mut file: File = File::create("database.json")?;
        file.write_all(data.as_bytes())?;
        Ok(())
    }

    fn load_from_file() -> std::io::Result<Self> {
        let file_content: String = read_to_string("database.json")?;
    ...
    HttpServer::new(move || {
        ...
        .wrap(
            ...
            .allowed_origin_fn(|origin, _req_head| {
                origin.as_bytes().starts_with(b"https://localhost") || origin == "null"
            })
